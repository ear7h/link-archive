    ...
    impl Token {
        ...
        pub fn issue(&self, secret : &[u8], exp_duration : time::Duration) -> Result<String> {

            let now = time::SystemTime::now();

            let iat = now.duration_since(time::UNIX_EPOCH)?
                .as_secs()
                .try_into()
                .unwrap();

            let exp = now.checked_add(exp_duration)
        ...
        pub fn validate(token : &str, secret : &[u8], iss : &str) -> Result<Self> {
            ...
            let tok : TokenFull = jwt::decode(
                ...
                &validation
            ).map_err(|err| err.into_kind())?.claims;
