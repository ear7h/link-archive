    ...
    macro_rules! db_method {
        ($name:ident (&$self:ident, $conn:ident, $($pname:ident : $ptype:ty),* ) -> $ret:ty $body:block ) => {
    ...
    impl Db {
        ...
        db_method!{insert_user(&self, conn, name : &str, password : &str) -> Result<()> {
            conn
                .prepare_cached("INSERT INTO users (name, password) VALUES (?, ?)")?
                .execute(rusqlite::params![name, password])
                .map_err(|err| {
                    if error_code_match(&err, ffi::ErrorCode::ConstraintViolation, 2067) {
        ...
        db_method!{get_user_by_name(&self, conn, username : &str) -> Result<models::User> {
            let mut stmt = conn
        ...
        db_method!{insert_link(&self, conn, user_id : u32, link : &str) -> Result<()> {
            conn
                .prepare_cached("INSERT INTO links (user_id, url) VALUES (?, ?)")?
                .execute(rusqlite::params![user_id, link])
                .map_err(|err| {
                    if error_code_match(&err, ffi::ErrorCode::ConstraintViolation, 1555) {
        ...
        db_method!{get_links(&self, conn, user_id : u32) -> Result<Vec<models::Link>> {
            let mut stmt = conn
